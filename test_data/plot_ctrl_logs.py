...
if False:
    ...
    y2 = axs[0].twinx()
    h1 = plot_param(y2, 'dreq', 'cornflowerblue')
    h2 = plot_param(axs[0], 'tm_rr', 'forestgreen')
    h3 = plot_param(axs[0], 'tm_rl', 'limegreen')
    add_legend(axs[0], h1+h2+h3)
    axs[0].set_ylabel('Torque [Nm]')
    y2.set_ylabel('[#]')

    y2 = axs[1].twinx()
    h1 = plot_param(axs[1], 'omega_fr', 'cornflowerblue')
    h2 = plot_param(axs[1], 'omega_fl', 'cyan')
    h3 = plot_param(axs[1], 'omega_rr', 'limegreen')
    h4 = plot_param(axs[1], 'omega_rl', 'forestgreen')
    h5 = plot_param(y2, 'steer', 'purple')
    add_legend(axs[1], h1+h2+h3+h4+h5)
    axs[1].set_ylabel('rad/sec')
    y2.set_ylabel('Sex. Degrees [$^\circ$]')
    y2.set_ylim([-180, 180])
